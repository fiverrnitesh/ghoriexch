...
const reducers = {
  registerNewPlayer: (
    ...
    const existingIndex = state.players.findIndex((p) => p.colour === action.payload.colour);
    ...
    if (existingIndex !== -1) {
      state.players[existingIndex] = newPlayer;
    } else {
  ...
  setPlayerSequence: (
    ...
    state.currentPlayerColour = state.playerSequence[0] ?? 'blue';
  },
